# Makefile for Hello World C program with plugin support
# Provides both standard hello and plugin-enabled versions

# Compiler and flags configuration
CC ?= gcc
CLANG ?= clang
CFLAGS ?= -Wall -Wextra -std=c99
OPTFLAGS = -O2
DEBUGFLAGS = -g
# STRICT_FLAGS: includes -Werror to fail on any warnings, ensuring code quality
STRICT_FLAGS = -Wpedantic -Wformat=2 -Wconversion -Wsign-conversion -Werror
PLUGIN_FLAGS = -DENABLE_PLUGINS -ldl

# Combined flag sets for cleaner target definitions
STRICT_CFLAGS = $(CFLAGS) $(STRICT_FLAGS)
DEBUG_CFLAGS = $(CFLAGS) $(DEBUGFLAGS)
OPT_CFLAGS = $(CFLAGS) $(OPTFLAGS)
PLUGIN_CFLAGS = $(CFLAGS) $(PLUGIN_FLAGS)

# Source files and targets
SOURCE = hello.c
PLUGIN_SOURCES = plugin_loader.c
TARGET = hello
DEBUG_TARGET = hello_debug
CLANG_TARGET = hello_clang
STRICT_TARGET = hello_strict
PLUGIN_TARGET = hello_plugin

# Default target (optimized build without plugins for backward compatibility)
all: $(TARGET)

# Standard optimized build (no plugins)
$(TARGET): $(SOURCE)
	$(CC) $(OPT_CFLAGS) -o $(TARGET) $(SOURCE)

# Debug build target (debug info without optimization)
debug: $(DEBUG_TARGET)

$(DEBUG_TARGET): $(SOURCE)
	$(CC) $(DEBUG_CFLAGS) -o $(DEBUG_TARGET) $(SOURCE)

# Strict compilation target (with -Werror for CI/quality assurance)
strict: $(STRICT_TARGET)

$(STRICT_TARGET): $(SOURCE)
	$(CC) $(STRICT_CFLAGS) -o $(STRICT_TARGET) $(SOURCE)

# Clang build target (with optimization)
clang: $(CLANG_TARGET)

$(CLANG_TARGET): $(SOURCE)
	$(CLANG) $(OPT_CFLAGS) -o $(CLANG_TARGET) $(SOURCE)

# Plugin-enabled build target
plugin: $(PLUGIN_TARGET)

$(PLUGIN_TARGET): $(SOURCE) $(PLUGIN_SOURCES)
	$(CC) $(PLUGIN_CFLAGS) -o $(PLUGIN_TARGET) $(SOURCE) $(PLUGIN_SOURCES)

# Plugin-enabled strict build for testing
plugin-strict: hello_plugin_strict

hello_plugin_strict: $(SOURCE) $(PLUGIN_SOURCES)
	$(CC) $(STRICT_CFLAGS) $(PLUGIN_FLAGS) -o hello_plugin_strict $(SOURCE) $(PLUGIN_SOURCES)

# Run the default binary
run: $(TARGET)
	./$(TARGET)

# Run the plugin-enabled binary
run-plugin: $(PLUGIN_TARGET)
	./$(PLUGIN_TARGET)

# Test target: run validation script (which will call make strict)
test:
	bash test/validate.sh

# Test target with quiet output for CI environments
test-quiet:
	bash test/validate.sh --quiet

# Validate target: run validation script on built binaries (compatibility with main branch)
validate: strict
	@chmod +x validate.sh
	@echo "=== Validating strict build ==="
	@./validate.sh ./$(STRICT_TARGET)

# Comprehensive validation target: test all build variants (for CI)
validate-all: all strict clang
	@chmod +x validate.sh
	@echo "=== Validating optimized build ==="
	@./validate.sh ./$(TARGET)
	@echo ""
	@echo "=== Validating strict build ==="
	@./validate.sh ./$(STRICT_TARGET)
	@echo ""
	@echo "=== Validating Clang build ==="
	@./validate.sh ./$(CLANG_TARGET)

# Plugin validation
validate-plugin: plugin-strict
	@chmod +x validate.sh
	@echo "=== Validating plugin build ==="
	@./validate.sh ./hello_plugin_strict

# Build sample plugins
plugins: $(PLUGIN_TARGET)
	$(MAKE) -C plugins all

# Test plugins with sample plugins loaded
test-plugins: plugins
	./$(PLUGIN_TARGET)

# Clean build artifacts
clean:
	rm -f $(TARGET) $(DEBUG_TARGET) $(CLANG_TARGET) $(STRICT_TARGET) 
	rm -f $(PLUGIN_TARGET) hello_plugin_strict
	rm -f *.exe *.out *.o
	$(MAKE) -C plugins clean

# Help target to show available commands
help:
	@echo "Available targets:"
	@echo "  all         - Build optimized hello (default, no plugins)"
	@echo "  debug       - Build with debug flags (-g)"
	@echo "  strict      - Build with strict warnings and -Werror"
	@echo "  clang       - Build with clang compiler"
	@echo "  plugin      - Build with plugin support enabled"
	@echo "  plugin-strict - Build plugin version with strict flags"
	@echo "  plugins     - Build sample plugins"
	@echo "  run         - Run the default binary"
	@echo "  run-plugin  - Run the plugin-enabled binary"
	@echo "  test        - Build strict and run (for CI/validation)"
	@echo "  test-quiet  - Same as test but with minimal output"
	@echo "  test-plugins - Build and test with sample plugins"
	@echo "  validate    - Validate strict build output and exit code"
	@echo "  validate-all - Validate all build variants"
	@echo "  validate-plugin - Validate plugin build"
	@echo "  clean       - Remove all build artifacts"
	@echo "  help        - Show this help message"

# Declare phony targets
.PHONY: all debug strict clang plugin plugin-strict plugins run run-plugin test test-quiet test-plugins validate validate-all validate-plugin clean help